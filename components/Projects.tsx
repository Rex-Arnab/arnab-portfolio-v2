  ...
  return (
    <section id="projects" className="py-24 bg-[#0a0a0a]">
      ...
      <div className="max-w-6xl mx-auto px-6">
        <FadeIn>
          <span className="text-xs font-mono text-[#2563eb] tracking-widest uppercase">Work</span>
          <h2 className="text-3xl sm:text-4xl font-bold text-white mt-2 mb-10">Selected Projects</h2>
        </FadeIn>
          ...
          {projects.map((p, i) => (
            <FadeIn key={p.title} delay={i * 0.08}>
              <motion.div
                ...
                className="bg-[#111] border border-[#1a1a1a] rounded-lg overflow-hidden h-full flex flex-col"
              >
                {p.image ? (
                  <button
                    onClick={() => setLightbox({ src: p.image!, alt: p.title })}
                    className="relative w-full h-40 bg-[#0d0d0d] overflow-hidden group/thumb block"
                  ...
                  >
                    ...
                    />
                    <div className="absolute inset-0 bg-[#0a0a0a]/20 group-hover/thumb:bg-[#0a0a0a]/40 transition-colors duration-200" />
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/thumb:opacity-100 transition-opacity duration-200">
                      <div className="bg-black/60 border border-[#333] rounded-full p-2">
                  ...
                  <div className="w-full h-40 bg-[#0d0d0d] border-b border-[#1a1a1a] flex items-center justify-center">
                    <div className="w-10 h-10 rounded bg-[#2563eb]/10 border border-[#2563eb]/20 flex items-center justify-center">
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <p className="text-[#666] text-xs leading-relaxed flex-1 mb-4">{p.desc}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {p.tags.map((t) => (
                      <span
                        key={t}
                        className="text-[10px] font-mono text-[#555] bg-[#0d0d0d] border border-[#1e1e1e] px-2 py-0.5 rounded"
                      >
                        {t}
                      </span>
                    ))}
